...
public class ExpenseOutputHandler implements IOutputHandler {
	...
	public String outputAll() throws SQLException {
		...
		while (resultSet.next()) {
			...
			builder.append(
					"  - " + stringUtility.postIncrementTo(resultSet.getString("name"), maxNameLength + gap, ' '));
			builder.append(stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("sum"), 2),
					maxSumLength + gap, ' '));
			builder.append("\u20AC");
			builder.newLine();
		}
	...
	public String outputAverage() throws SQLException {
		...
		if (resultSet.next()) {
			...
			monthCount = ChronoUnit.MONTHS.between(firstDate,
		...
		if (resultSet.next()) {
			...
			builder.newLine();
			builder.append(" - day   : " + stringUtility
					.preIncrementTo(stringUtility.formatDouble(dayAverage, 2) + "\u20AC", yearAverage.length(), ' '));
			builder.newLine();
			builder.append(" - month : " + stringUtility
					.preIncrementTo(stringUtility.formatDouble(monthAverage, 2) + "\u20AC", yearAverage.length(), ' '));
			builder.newLine();
	...
	public String outputCategory() throws SQLException {
		...
		while (resultSet.next()) {
			builder.append(" - "
					+ stringUtility.postIncrementTo(resultSet.getString("category"), maxCategoryLength + gap, ' '));
			builder.append(stringUtility.preIncrementTo(
					stringUtility.formatDouble(resultSet.getDouble("sum"), 2) + "\u20AC", maxSumLength + gap, ' '));
			builder.append(
					stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("percentage"), 2) + "%",
							maxPercentageLength + gap, ' '));
			builder.newLine();
		}
		...
		if (resultSet.next()) {
			...
			builder.append(stringUtility.preIncrementTo(
					stringUtility.formatDouble(resultSet.getDouble("total"), 2) + "\u20AC", length, ' '));
		}
		resultSet.close();
		return builder.build();
	}
	...
	@Command(tag = "month")
	public String outputMonth() throws InterruptedException, BadLocationException, UserCancelException, SQLException {
		...
		while (retry) {
			...
			dateString = stringUtility.parseDateString(
					dateString.isEmpty() ? LocalDate.now().format(DateTimeFormatter.ofPattern("dd.MM.uuuu"))
							: "01." + dateString);
		...
		while (resultSet.next()) {
			...
			builder.append(stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("value"), 2),
					gap - resultSet.getString("name").length(), ' '));
			builder.append("\u20AC");
			builder.newLine();
		}
		resultSet.close();
	...
	@Command(tag = "months")
	public String outputMonths() throws SQLException {
		StringUtility stringUtility = new StringUtility();
		ResultSet resultSet;
		String month = "";
		...
		while (resultSet.next()) {
			...
			list.add(" " + resultSet.getString("month") + " : " + stringUtility.preIncrementTo(
					stringUtility.formatDouble(resultSet.getDouble("value"), 2), map.get(month), ' ') + "\u20AC");
		}
