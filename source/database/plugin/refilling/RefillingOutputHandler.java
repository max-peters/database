...
public class RefillingOutputHandler implements IOutputHandler {
	...
	@Command(tag = "all") public String outputAll() throws SQLException {
		...
		resultSet = database.execute(SQLStatements.REFILLING_SELECT_ALL);
		while (resultSet.next()) {
			String incrementedDistance = stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("distance"), 1), maxDistanceLength, ' ');
			String incrementedRefuelAmount = stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("refuelAmount"), 1), maxRefuelAmountLength, ' ');
			String incrementedConsumption = stringUtility.preIncrementTo(stringUtility.formatDouble(resultSet.getDouble("average"), 1), maxConsumptionLength, ' ');
